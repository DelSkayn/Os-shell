...
bool match_command(struct CommandBuffer * cb,char ** cwd,const char ** path){
    if(strcmp(cb->command_buffer,"exit") == 0){
        ...
        parse_command(buff,cb->command_buffer," ");
        if(strcmp(buff[0],"cd") == 0){
            ...
            if(found_pipe){
                ...
                char * full_path = command_full_path(full_path_buff,path,buff[0]);
                if(!full_path){
                    return true;
                }
                pid_t pid = fork();
                {
                    ...
                    if(pid == 0){//parent
                        ...
                        char * sub_full_path = command_full_path(sub_buff,path,buff[pos+1]);
                        if(!sub_full_path){
                            exit(1);
                        }
                        int pipes[2];
                        ...
                        pid_t pid2 = fork();
                        {
                            if(pid2 == 0){//CHILD
                                ...
                                close(pipes[0]);
                                if(execv(sub_full_path,&buff[pos+1]) == -1)
                                    printf("%s\n",strerror(errno));
                                exit(0);
                            }else{ //PARENT
                                close(pipes[0]);
